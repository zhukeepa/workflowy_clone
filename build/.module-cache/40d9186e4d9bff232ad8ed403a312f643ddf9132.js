...
function getPathOfLastChild(todoItem) {
  var childNum = todoItem.get("children").size; 
  if (childNum == 0)
    return L([]);
  else {
    return L([childNum-1]).concat(getPathOfLastChild(todoItem.get("children").get(childNum-1)));
  }
}
...
function oneUp(path, todos) { 
  ...
  {
    return newPath.concat(getPathOfLastChild(nodeAtNewPath));
  } 
  else
    return newPath; 
}
