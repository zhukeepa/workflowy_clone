...
function mergeWithAboveAtPath(xs, path, text) { 
  return operateAtPath(xs, path, function(l, p) { 
    if (p.size == 1) {
      ...
      if (cInd > 0) { 
        var tmp = mergeWithAboveAtPath(parent.get("children"), L([cInd]), text);
      ...
      var newNode = M({ text: parent.get("text"), children: children.splice(0,1) });
      return [l.splice(pIndex, 1, newNode), L([pInd])];
    }
  }, 2);
}
