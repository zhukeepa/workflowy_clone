...
Array.prototype.insertAfter = function(val, pos) {
  if (pos < self.length - 1)
    return self.splice(pos + 1, val); 
  return self.concat([val]);
}

Tree.prototype.insertBelowFromAbsolutePath = function(text, path) { 
  //javascript error-handling?? 
  var split_path = /\/(\d*)(.*)/.exec(path); 
  if (!split_path) { 
    console.error("Invalid path format"); 
    return; 
  }

  var index = Number(split_path[1]); 
  var rem_path = split_path[2];

  if (rem_path == '') {
    var newTree = new Tree(text, []); 
    return self.insertAfter(newTree, index); 
  }
