...
function operateAtPath(xs, path, flatOperator) { 
  if (path.size == 0) {
  ...
  var tmp = operateAtPath(xs.get(index).get("children"), restOfPath, flatOperator); 
  var newChild = tmp[0]; 
  var newPath  = tmp[1]; 
...
function operateAtPath(xs, path, semiflatOperator) { 
  if (path.size == 0) {
    throw new Error("Path must have positive length.")
  }
  ...
  var index = path.get(0);
  if (path.size <= 2) {
    return semiflatOperator(xs, index); 
  }
