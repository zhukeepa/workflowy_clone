...
var EditorComponent = React.createClass({displayName: "EditorComponent",
  ...
  setFocusFromAbsolutePath: function(path, list) { 
    ...
    console.log(React.findDOMNode(list.props.items.get(index)));
    if (path.size == 1) { 
      console.log(React.findDOMNode(list.props.items.get(index)).value);
      return React.findDOMNode(list.props.items.get(index)).value; 
    }
    
    var restOfPath = path.slice(1); 
    this.setFocusFromAbsolutePath(restOfPath, list.props.items.get(index));
  },
