...
var EditorComponent = React.createClass({displayName: "EditorComponent",
  ...
  setFocusFromAbsolutePath: function(path, list) { 
    ...
    console.log(path.toString(), list.props);//, list);
debugger;
    var index = path.get(0); 
    if (path.size == 1) { 
      console.log(React.findDOMNode(list.props.children.get(index).value));
      return React.findDOMNode(list.props.children.get(index)).value; 
    }
    
    var restOfPath = path.slice(1); 
    this.setFocusFromAbsolutePath(restOfPath, list.children.get(index));
  },
