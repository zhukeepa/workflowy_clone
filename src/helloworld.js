...
function insertAtPath(xs, val, path) { 
  ...
  var childPrime = child.set("children", insertAtPath(child.get("children"), val, restOfPath)); 
...
var EditorComponent = React.createClass({
  ...
  render: function () { 
    return <TodoListComponent items={this.state.todos} path={L([])} />
  }
})
...
var TodoListComponent = React.createClass({
  render: function() { 
    ...
    for (var i = 0; i < this.props.items.length; i++) {
      ...
      ret.push(<TodoItemComponent text={t.get("text")} path={path} />);
      if (t.get("children").size > 0)
        ret.push(<TodoListComponent items={t.get("children")} path={path} />);
    }
    return (<ul>{ret}</ul>);
  } 
});
...
var TodoItemComponent = React.createClass({
  render: function() {
    return (
      <li>
        <span onClick={this.props.onClick}>{this.props.path.toString()}</span>
      </li>
    ); 
  } 
});
